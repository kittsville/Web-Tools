...
const cleanUrl = (input) => {
  try {
    ...
    const allowedParams = hostToAllowedParamsMap[url.host] ? hostToAllowedParamsMap[url.host] : [];
...
const hostToAllowedParamsMap = Object.assign({}, ...queryParamAllowlist.flatMap(entry => entry.hosts.map(host => ({[host] : entry.params}))));
